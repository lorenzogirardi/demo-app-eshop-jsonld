...
class ProductRetriever(BaseProductRetriever):
    ...
    def find_products_with_sql(self, sql_query: str) -> list[Product]:
        ...
        try:
            expression = sqlglot.parse_one(cleaned_query)
            if isinstance(expression, sqlglot.exp.Select):
                select_columns = [s.name.lower() if hasattr(s, 'name') else str(s).lower() for s in expression.find_all(sqlglot.exp.Column)]
                if not any(c == 'id' or c.endswith('.id') for c in select_columns):
                    expression = expression.select(sqlglot.exp.Column(this=sqlglot.exp.to_identifier("id"), table=sqlglot.exp.to_identifier("catalog_product")))
                    cleaned_query = expression.sql()
