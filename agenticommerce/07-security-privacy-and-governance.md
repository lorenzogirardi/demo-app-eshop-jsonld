...
| T16 | **Use of the AI as a general chatbot** (cost, brand, abuse) | `/api/ai/chat`, `/api/ai/search` | Scope guard before any answer; scope rule in the agent prompt and in the direct prompts | Classifier can be evaded; fails open; rate limit is per IP | Medium |
